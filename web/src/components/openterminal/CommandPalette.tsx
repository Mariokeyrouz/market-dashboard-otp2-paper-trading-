// Ported from OpenTerminal's web/components/CommandPalette.tsx.
// apiGet path prefixed to /api/openterminal/... (see the plan).
"use client";
...
export default function CommandPalette() {
  const open = useOpenTerminal((s) => s.commandOpen);
  const setOpen = useOpenTerminal((s) => s.setCommandOpen);
  ...
  const { data: results = [] } = useQuery({
    ...
    queryFn: () => apiGet<SearchResult[]>(`/api/openterminal/search?q=${encodeURIComponent(query)}`),
