// Ported from OpenTerminal's web/components/Workspace.tsx. This project's
// react-grid-layout is v2 (a different major than OpenTerminal's pinned v1 —
// see the plan's Dependencies note), so this imports the package's `/legacy`
// entry point, which is a 100% runtime-API-compatible v1 shim — the grid
// logic below is otherwise untouched.
"use client";
...
import { useOpenTerminal, type WidgetInstance } from "@/store/openterminal";
...
function SymbolTag({ widget, activeSymbol }: { widget: WidgetInstance; activeSymbol: string }) {
  const setWidgetSymbol = useOpenTerminal((s) => s.setWidgetSymbol);
...
export default function Workspace() {
  const widgets = useOpenTerminal((s) => s.widgets);
  const layout = useOpenTerminal((s) => s.layout);
  const setLayout = useOpenTerminal((s) => s.setLayout);
  const removeWidget = useOpenTerminal((s) => s.removeWidget);
  const toggleLinked = useOpenTerminal((s) => s.toggleLinked);
