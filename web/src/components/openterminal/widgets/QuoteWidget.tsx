// Ported from OpenTerminal's web/components/widgets/QuoteWidget.tsx.
...
export default function QuoteWidget({ widget }: { widget: WidgetInstance }) {
  ...
  const { data, error } = useQuery({
    ...
    queryFn: async () => (await apiGet<Quote[]>(`/api/openterminal/quotes?symbols=${symbol}`))[0],
  ...
  const { data: shortVol } = useQuery({
    ...
    queryFn: () => apiGet<ShortVolume | null>(`/api/openterminal/short-volume/${symbol}`),
    staleTime: 3_600_000,
  });
