// Ported from OpenTerminal's web/components/widgets/CalendarWidget.tsx.
// apiGet paths prefixed to /api/openterminal/...
"use client";

import { useQuery } from "@tanstack/react-query";
import { Fragment, useMemo, useState } from "react";
import { apiGet, fmt, pctClass } from "@/lib/openterminal/api";
...
function EconomicTab() {
  ...
  const { data = [], isLoading, error } = useQuery({
    ...
    queryFn: () => apiGet<EconEvent[]>("/api/openterminal/econ-calendar"),
...
function EarningsHistoryRows({ symbol }: { symbol: string }) {
  const { data = [], isLoading, error } = useQuery({
    ...
    queryFn: () => apiGet<EarningsHistoryRow[]>(`/api/openterminal/earnings-history/${symbol}`),
    staleTime: 3_600_000,
  });
...
function EarningsTab() {
  ...
  const { data = [], isLoading, error } = useQuery({
    ...
    queryFn: () => apiGet<EarningsEntry[]>(`/api/openterminal/calendar?symbols=${watchlist.join(",")}`),
