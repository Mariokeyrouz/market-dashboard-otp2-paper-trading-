// Ported from OpenTerminal's web/components/widgets/CryptoWidget.tsx.
// apiGet paths prefixed to /api/openterminal/...
"use client";

import { useQuery } from "@tanstack/react-query";
import { apiGet, fmt, fmtBig, pctClass } from "@/lib/openterminal/api";
...
export default function CryptoWidget() {
  ...
  const { data = [], error } = useQuery({
    queryKey: ["crypto"],
    queryFn: () => apiGet<CryptoRow[]>("/api/openterminal/crypto"),
    refetchInterval: 1_000,
  });
  const { data: global } = useQuery({
    queryKey: ["crypto-global"],
    queryFn: () => apiGet<GlobalStats>("/api/openterminal/crypto/global"),
