// Ported from OpenTerminal's web/components/widgets/AiWidget.tsx.
// apiGet/apiPost paths prefixed to /api/openterminal/...
"use client";

import { useMutation } from "@tanstack/react-query";
import { useRef, useState } from "react";
import { apiGet, apiPost, type Quote } from "@/lib/openterminal/api";
...
export default function AiWidget() {
  ...
  const chat = useMutation({
    mutationFn: async (userText: string) => {
      ...
      try {
        context = { activeSymbol, quote: (await apiGet<Quote[]>(`/api/openterminal/quotes?symbols=${activeSymbol}`))[0] };
      } catch {
        // context is best-effort
      }
      const next = [...messages, { role: "user" as const, content: userText }];
      const res = await apiPost<{ text: string }>("/api/openterminal/ai/chat", { messages: next, context });
