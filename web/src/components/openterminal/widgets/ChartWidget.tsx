// Ported from OpenTerminal's web/components/widgets/ChartWidget.tsx.
...
import { apiGet, fmt, fmtBig, type Candle } from "@/lib/openterminal/api";
import { sma, ema, vwap, rsi, macd, bollinger, type Point } from "@/lib/openterminal/indicators";
import { useWidgetSymbol, type WidgetInstance } from "@/store/openterminal";
...
export default function ChartWidget({ widget }: { widget: WidgetInstance }) {
  ...
  const { data: candles, error } = useQuery({
    ...
    queryFn: () => apiGet<Candle[]>(`/api/openterminal/history/${symbol}?range=${range}`),
