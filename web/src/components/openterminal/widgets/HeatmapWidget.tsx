// Ported from OpenTerminal's web/components/widgets/HeatmapWidget.tsx.
// apiGet path prefixed to /api/openterminal/...
/* eslint-disable @typescript-eslint/no-explicit-any -- d3's hierarchy/treemap
   generics don't carry the leaf datum type through .data()/.join() callbacks
   without a lot of ceremony; kept loosely typed same as upstream. */
"use client";
...
export default function HeatmapWidget() {
  ...
  const { data, error } = useQuery({
    queryKey: ["heatmap"],
    queryFn: () => apiGet<Cell[]>("/api/openterminal/heatmap"),
