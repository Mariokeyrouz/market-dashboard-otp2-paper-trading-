// Ported from OpenTerminal's web/components/widgets/ScreenerWidget.tsx.
...
export default function ScreenerWidget() {
  ...
  const { data: sectors = [] } = useQuery({
    queryKey: ["sectors"],
    queryFn: () => apiGet<string[]>("/api/openterminal/sectors"),
    staleTime: 600_000,
  });
  ...
  const { data = [], isLoading, error } = useQuery({
    ...
    queryFn: () => apiGet<Row[]>(`/api/openterminal/screener?${params}`),
