// Ported from OpenTerminal's web/components/widgets/OptionsWidget.tsx.
// apiGet path prefixed to /api/openterminal/...
"use client";

import { useQuery } from "@tanstack/react-query";
import { useState } from "react";
import { apiGet, fmt, fmtBig } from "@/lib/openterminal/api";
...
export default function OptionsWidget({ widget }: { widget: WidgetInstance }) {
  ...
  const { data, error, isLoading } = useQuery({
    ...
    queryFn: () => apiGet<Chain>(`/api/openterminal/options/${symbol}${expiry ? `?expiry=${encodeURIComponent(expiry)}` : ""}`),
