...
export default function SectorPerformance() {
  ...
  return (
    <div style={TILE}>
      ...
      {!compact && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 84px 46px 46px 46px", gap: 8, marginBottom: 3 }}>
          <span />
          <span />
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: compact ? 3 : 5, flex: 1, minHeight: 0, overflow: "hidden" }}>
        {rows.map((r) => (
          <div
            ...
            style={{
              display: "grid",
              gridTemplateColumns: compact ? "1fr 70px 44px" : "1fr 84px 46px 46px 46px",
              gap: 8,
              alignItems: "center",
            }}
          >
            ...
            {!compact && (
              <>
                <span style={{ fontFamily: MONO, fontSize: 11.5, textAlign: "right", color: r.chg1wColor }}>{r.chg1w}</span>
                <span style={{ fontFamily: MONO, fontSize: 11.5, textAlign: "right", color: r.chg1mColor }}>{r.chg1m}</span>
              </>
            )}
          </div>
        ))}
