...
export default function EquityYieldCurve() {
      ...
      <svg viewBox="0 0 460 210" style={{ width: "100%", flex: 1, minHeight: 0, display: "block" }} preserveAspectRatio="none">
        {c.ticks.map((t, i) => (
          <g key={i}>
            ...
            <text x={37} y={t.ty} fontFamily="var(--font-plex-mono), monospace" fontSize={9.5} fill="var(--faint)" textAnchor="end">
              {t.label}
            </text>
          </g>
        ))}
        ...
        {c.pts.map((p) => (
          <g key={p.t}>
            ...
            <text x={p.x} y={p.vy} fontFamily="var(--font-plex-mono), monospace" fontSize={9} fontWeight={600} fill="var(--gold-deep)" textAnchor="middle">
              {p.v}
            </text>
            <text x={p.x} y={190} fontFamily="var(--font-plex-mono), monospace" fontSize={9.5} fill="var(--muted)" textAnchor="middle">
              {p.t}
            </text>
          </g>
        ))}
