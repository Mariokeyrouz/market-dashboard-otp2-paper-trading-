// Ported from OpenTerminal's web/store/terminal.ts, renamed useTerminal ->
// useOpenTerminal so it doesn't clash with this app's own useDashStore.
"use client";
...
export const useOpenTerminal = create<OpenTerminalState>()(
  persist(
    (set) => ({
      activeSymbol: "AAPL",
...
export function useWidgetSymbol(widget: WidgetInstance): string {
  const active = useOpenTerminal((s) => s.activeSymbol);
