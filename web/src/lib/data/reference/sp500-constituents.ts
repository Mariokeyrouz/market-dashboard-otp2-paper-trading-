/**
 * S&P 500 ticker universe for the Movers tile (ranks live day-change across
 * the full index rather than a curated subset). Seeded from the header row
 * of `momentum_daily_prices.csv` at the repo root — the same maintained
 * constituent list the Streamlit side already tracks. Symbols are already in
 * Yahoo Finance format (e.g. "BRK-B", "BF-B").
 *
 * S&P 500 membership changes a few times a year — refresh by re-pulling that
 * CSV's header row when movers look off (e.g. a recently-removed ticker
 * stops returning quotes).
 */
