// Ported near-verbatim from OpenTerminal's server/src/providers/yahoo.ts.
/* eslint-disable @typescript-eslint/no-explicit-any -- parsing Yahoo's unofficial,
   undocumented JSON responses; kept loosely typed same as upstream rather than
   guessing exact response shapes for an API with no published schema. */
const UA =
