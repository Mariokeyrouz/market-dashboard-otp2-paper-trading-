// Ported near-verbatim from OpenTerminal's server/src/cache.ts. In-process
// memoization for the market data routes below — same semantics (TTL cache +
// a stale fallback for provider outages), just living inside a Next.js
// serverless/edge process instead of a long-running Express one. Note this
// means the cache resets per cold start / per instance, not truly global.
